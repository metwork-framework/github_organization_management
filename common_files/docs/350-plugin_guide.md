...
```
{{REPO|upper}}_CURRENT_PLUGIN_CUSTOM_FOO=value1
{{REPO|upper}}_CURRENT_PLUGIN_CUSTOM_BAR=value2
```
...
{{ nested_raw() }}
```
{{ "{{" + REPO|upper + "_HOME}}" }}/bin/cronwrap.sh --lock --log-capture-to your_command.log -- plugin_wrapper {{ "{{" + REPO|upper + "_CURRENT_PLUGIN_NAME}}" }} --
```

It will:

- replace automatically `{{ "{{" + REPO|upper + "_HOME}}" }} }}`, `{{ "{{" + REPO|upper + "_CURRENT_PLUGIN_NAME}}" }}`, `{{ "{{" + REPO|upper + "_CURRENT_PLUGIN_DIR + "}}" }}`, `{{ "{{" + REPO|upper + "_CURRENT_PLUGIN_CUSTOM_*}}" }}` variables (to avoid hardcoding things) ([full list of available variables](../850-reference/700-env/))
...
??? question "How to debug?"
    ...
    for all plugins and `{{REPO}}` module.
...
as `{{REPO}}` user (or in corresponding MetWork env for custom installations).
...
`.plugin` file in `/etc/metwork.config.d/{{REPO}}/plugins/` directory.

Then, restart the corresponding module by restarting the corresponding service with
your favorite system services manager or with `/etc/rc.d/init.d/metwork restart {{REPO}}` (as root user).
...
These keys are available in `${MFMODULE_RUNTIME_HOME}/config/plugins/{plugin_name}.ini`. So for example in `~{{REPO}}/config/config.ini` (for a standard `{{REPO|upper}}` module). By default, all keys
...
If you create a file in `/etc/metwork.config.d/{{REPO}}/plugins/{plugin_name}.ini` with a configuration part you want to override:
...
!!! note
    ...
    the `/etc/metwork.config.d/{{REPO}}/plugins/{plugin_name}.ini` configuration
...
!!! warning
    It can't work if the plugin hardcodes its name in its code. Developers should use `{{REPO|upper}}_CURRENT_PLUGIN_NAME` env var to avoid that.
